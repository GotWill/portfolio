                    ...
                    itemsSkilTec.map((skil) => {
                        ...
                        return (
                            <div key={skil.id} className="w-48 h-40 md-mobile:w-40 px-3 bg-secondary rounded-3xl flex justify-center items-center flex-col gap-4  overflow-hidden text-white relative">
                                ...
                                <span className={`border-after before:absolute before:left-[4.5rem] before:animate-spin-slow before:w-20 before:h-64 ${skil.bg === 'html' ? 'before:bg-[#fc4503]' : ''} ${skil.bg === 'css' ? 'before:bg-[#0328fc]' : ''} ${skil.bg === 'js' ? 'before:bg-[#fefe01]' : ''} ${skil.bg === 'typescript' ? 'before:bg-[#007acc]' : ''} ${skil.bg === 'php' ? 'before:bg-[#4e5b98]' : ''} ${skil.bg === 'php' ? 'before:bg-[#4e5b98]' : ''} ${skil.bg === 'react' || skil.bg === 'react-native' ? 'before:bg-[#61dafb]' : ''} ${skil.bg === 'next' ? 'before:bg-[#FFF]' : ''} ${skil.bg === 'styled-components' ? 'before:bg-[#9f70b8]' : ''} ${skil.bg === 'bootstrap' ? 'before:bg-[#712cf9]' : ''} ${skil.bg === 'bootstrap' ? 'before:bg-[#38bdf8]' : ''} ${skil.bg === 'tailwind' ? 'before:bg-[#712cf9]' : ''} ${skil.bg === 'mysql' ? 'before:bg-[#3e6e93]' : ''} ${skil.bg === 'postgresql' ? 'before:bg-[#336791]' : ''} ${skil.bg === 'github' ? 'before:bg-[#de4736]' : ''}  ${skil.bg === 'wordpress' ? 'before:bg-[#288eb0]' : ''} before:top-[-40%] after:absolute after:inset-[5px] bg-gradient-css after:rounded-3xl `}></span>
                            </div>
